...
export class AdminComponent implements OnInit {
  ...
  // Loading individuel par item
  actionLoading: { [id: number]: boolean } = {};

  // Modal ban/déban
  pendingBanUserId: number | null = null;
  pendingBanUsername = '';
  pendingBanIsBanned = false;
  banLoading = false;
  ...
  requestUpdateReportStatus(report: AdminReport, status: 'REVIEWED' | 'DISMISSED'): void {
    this.pendingReportId = report.id;
    this.pendingReportAction = status;
    this.pendingReportedUsername = report.reportedUsername;
  }

  closeReportModal(): void {
    this.pendingReportId = null;
    this.pendingReportAction = null;
    this.pendingReportedUsername = '';
  }

  confirmUpdateReportStatus(): void {
    if (!this.pendingReportId || !this.pendingReportAction) return;
    const reportId = this.pendingReportId;
    const status = this.pendingReportAction;
