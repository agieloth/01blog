...
export class WebSocketService implements OnDestroy {
      ...
      try {
        ...
        this.stompClient.connect(
          ...
          () => {
            ...
            this.stompClient.subscribe('/topic/posts', (msg: any) => {
              this.ngZone.run(() => this.postEvents$.next(JSON.parse(msg.body)));
            });

            this.stompClient.subscribe('/topic/likes', (msg: any) => {
              this.ngZone.run(() => this.likeEvents$.next(JSON.parse(msg.body)));
            });

            this.stompClient.subscribe('/topic/follows', (msg: any) => {
              this.ngZone.run(() => this.followEvents$.next(JSON.parse(msg.body)));
            });

            if (this.currentUserId) {
              this.stompClient.subscribe(
                `/topic/notifications/${this.currentUserId}`,
                (msg: any) => this.ngZone.run(() => this.notificationEvents$.next(JSON.parse(msg.body)))
              );
            }
  ...
  private _subscribeCommentTopic(postId: number): void {
    ...
    const sub = this.stompClient.subscribe(
      `/topic/comments/${postId}`,
      (msg: any) => this.ngZone.run(() => this.commentEvents$.get(postId)?.next(JSON.parse(msg.body)))
    );
    this.stompCommentSubs.set(postId, sub);
  }
